...
try:
    ...
    ActType = TypeVar("ActType")  # type: ignore[misc, assignment]
...
class State(ABC):
    ...
    def distance(self, other: object) -> float:
        ...
        注意：不要使用 `other == self`，因为某些 dataclass 可能包含 numpy.ndarray，
        dataclass 默认的逐字段 __eq__ 会触发 ndarray 的逐元素比较并报错
        (ValueError: truth value of an array is ambiguous)。
        这里统一用 hash 判等；若 other 不可 hash，则视为不相等。
        """
        try:
            return 0.0 if hash(other) == hash(self) else float("inf")
        except Exception:
            return float("inf")
