...
from .pddl_state_manager import snapshot_problem_from_state
...
class HybridParams:
    ...
    # If False, skip all Julia info-POMDP triggers and execute the classical plan directly.
    use_pomdp: bool = True
...
class HybridController:
    ...
    def replan_from_real_state(self) -> None:
        """
        Recovery replanning:
        - snapshot the real (ground-truth) UP state into a fresh planning problem
        - solve from that snapshot to finish the remaining task

        Uses `snapshot_problem_from_state` (UP-level snapshot), then reparses the written PDDL
        via `ClassicalPlanner.parse_problem` so we benefit from the planner's domain sanitization and
        compilation fallback logic.
        """
        try:
            snap = snapshot_problem_from_state(self.env.problem, self.env.state)
            with tempfile.TemporaryDirectory(prefix="single_pomdp_recovery_") as td:
                snap_path = Path(td) / "snapshot_problem.pddl"
                PDDLWriter(snap).write_problem(str(snap_path))
                prob = self.planner.parse_problem(str(snap_path))
            self.plan = self.planner.solve(prob)
        except RuntimeError as e:
            self.plan = []
            self._last_plan_error = f"recovery_plan_failed: {e}"
    ...
    def step(self) -> dict[str, Any]:
        ...
        for _ in range(max(1, int(self.params.macro_max_steps))):
            ...
            if self.params.use_pomdp and aname == "floor_pick_part":
                part = args[1]
            ...
            if aname == "floor_pick_part":
                ...
                res = self.env.apply_action(a)
            if not res.ok:
                # If we are running without the info-POMDP layer, stop immediately on any execution failure.
                if not self.params.use_pomdp:
                    events.append({"action": a, "ok": False, "reason": res.reason})
                    return {"done": True, "reason": f"execution_failed: {res.reason}", "events": events}
