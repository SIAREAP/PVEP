...
|---|---|---|---|---:|---|---|
| Fig. 2d/e five-method score comparison | Row-specific natural-language or PDDL foundation-model candidate in `results/ariac/nominal_ablation_per_scenario.csv` | NR in the legacy aggregate | Row-specific: none, generic re-proposal, `V_error`, or full PVEP; exact model identifier NR | Target 900 s/scenario; realised per-row budget NR | Attempt-level status is not retained in this aggregate | Scenario-normalized score |
